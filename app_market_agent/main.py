...
class AppMarketAgent:
    ...
    def generate_report_content(self, store_results: list) -> str:
        report = f"## 📈 일일 유망 앱 마켓 분석 리포트 ({datetime.now().strftime('%Y-%m-%d')})\n\n"
        report += "본 리포트는 니치 마켓, 수익 모델, 1인 개발 가능성을 기준으로 필터링된 잠재력 높은 인디 iOS 앱 목록입니다.\n\n"
        
        report += "### 🏆 엄선된 유망 타겟 앱\n"
        if not store_results:
            report += "조건을 만족하는 앱이 없습니다.\n\n"
        else:
            for i, result in enumerate(store_results, 1):
                ...
                report += f"   - 출처(검색 키워드): {app.get('source_keyword', 'Unknown')}\n"
                report += f"   - 설명 요약: {app.get('description', '').replace(chr(10), ' ')[:150]}...\n"
                report += f"   - **선정 사유 (LLM 평가):**\n"
                report += f"     - 니치 마켓: {eval_reason.get('niche_market', {}).get('reason', '')}\n"
                report += f"     - 수익 모델: {eval_reason.get('revenue_model', {}).get('reason', '')}\n"
                report += f"     - 기능 단순성: {eval_reason.get('simplicity', {}).get('reason', '')}\n\n"
    ...
    def send_email_report(self, markdown_content: str):
        ...
        msg['Subject'] = f"[App Market Analyzer] 일일 분석 리포트 - {datetime.now().strftime('%Y-%m-%d')}"
